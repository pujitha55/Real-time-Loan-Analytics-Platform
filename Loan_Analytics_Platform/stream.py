...
class LoanEventsStream:
    """Reads streaming loan events from Kafka, applies transformations, and saves to Iceberg sink."""
    ...
    def _extract_payload(self, df: DataFrame) -> DataFrame:
        """Deserializes Kafka topic value from Avro (skip Confluent's 5-byte wire format)."""
        return (
    ...
    def _transform(df: DataFrame) -> DataFrame:
        ...
        max_late_arriving_events_date = (
            datetime.datetime.now(datetime.UTC).date()
            - datetime.timedelta(days=LATE_ARRIVING_EVENTS_THRESHOLD_DAYS)
        )

        return (
            df.dropDuplicates(["loan_id"])
            .withColumn("is_high_risk", col("amount") > 10000)
            .withColumn("loan_date", to_date(from_unixtime("timestamp")))
            .where(col("loan_date") >= lit(max_late_arriving_events_date))
            .select(final_fields)
        )
    ...
    def _upsert_to_sink(self, df: DataFrame, batch_id: int) -> None:
        ...
        self._spark_session.sql(f"""
            ...
            ON existing.loan_id = incoming.loan_id
                AND existing.loan_date >= DATE_ADD(CURRENT_DATE(), -{LATE_ARRIVING_EVENTS_THRESHOLD_DAYS})
    ...
    def _create_sink_table_if_not_exists(self, output_table: str, df: DataFrame) -> None:
        if not Catalog(self._spark_session).tableExists(tableName=output_table):
            ...
            empty_batch_df.writeTo(output_table).partitionedBy(col("loan_date")).create()
